...
[ApiController]
[Authorize]
public sealed class LookupController(ISender sender) : BaseController
{
    ...
    public async Task<IActionResult> Create(string lookupType, CreateLookupRequest request)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(lookupType);

        var response = await sender
            .Send(new CreateLookupCommand(lookupType, request, GetUserId()))
            .ConfigureAwait(false);
    ...
    public async Task<IActionResult> Update(string lookupType, int id, UpdateLookupRequest request)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(lookupType);

        var response = await sender
            .Send(new UpdateLookupCommand(lookupType, id, request, GetUserId()))
            .ConfigureAwait(false);
    ...
    public async Task<IActionResult> Delete(string lookupType, int id)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(lookupType);

        var response = await sender
            .Send(new DeleteLookupCommand(lookupType, id, GetUserId()))
            .ConfigureAwait(false);
